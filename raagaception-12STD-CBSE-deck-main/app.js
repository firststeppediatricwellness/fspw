...
function initializeElements() {
    ...
    elements.sidebarToggle = document.getElementById('sidebar-toggle'); // May be null if removed
...
async function loadDeckData() {
    try {
        ...
        // Check if DECK_DATA is available
        if (typeof DECK_DATA !== 'undefined') {
            updateLoadingStatus('Loading embedded deck data...');
            state.deckData = DECK_DATA;

            updateLoadingStatus('Filtering invalid cards...');
            cleanDeckData(state.deckData);
...
function isValidCard(note) {
    if (!note || !note.fields || note.fields.length === 0) {
        return false;
    }

    // Check if any field has actual text content (not just images)
    for (let field of note.fields) {
        if (!field) continue;

        // Remove HTML tags to check text content
        const textOnly = field.replace(/<[^>]*>/g, '').trim();

        // Check if it's just a GUID (various formats)
        const isGUID = /^[a-f0-9]{32}(-[a-z]{2,3}-\d+)?$/i.test(textOnly) ||
            /^-[a-z]{2,3}-\d+$/i.test(textOnly);

        if (isGUID) {
            continue; // Skip this field, check next one
        }

        // Require meaningful text content (more than just whitespace or very short text)
        if (textOnly.length > 3) {
            return true;
        }
    }

    // If we get here, the card has no meaningful text content
    return false;
}

function cleanDeckData(deck) {
    // Remove invalid notes from current deck
    if (deck.notes && deck.notes.length > 0) {
        deck.notes = deck.notes.filter(note => isValidCard(note));
    }

    // Recursively clean children
    if (deck.children && deck.children.length > 0) {
        deck.children.forEach(child => cleanDeckData(child));
    }
...
function processCards(deck, path = []) {
    ...
    if (deck.notes && deck.notes.length > 0) {
        deck.notes.forEach(note => {
            // Validate card has actual content
            if (!isValidCard(note)) {
...
function updateFooterStats() {
    // Find specific decks and update footer stats
    const deckMap = {
        'Universally Important Stuff': 'stat-important',
        'PCM Level 1': 'stat-pcm1',
        'PCM Level 2': 'stat-pcm2',
        'BIO Level 1': 'stat-bio1',
        'BIO Level 2': 'stat-bio2',
        'JEE': 'stat-jee'
    };

    function findDeckByName(deck, targetName) {
        if (deck.name === targetName) {
            return deck;
        }
        if (deck.children) {
            for (let child of deck.children) {
                const found = findDeckByName(child, targetName);
    ...
    // Update each stat
    for (let [deckName, elementId] of Object.entries(deckMap)) {
        const foundDeck = findDeckByName(state.deckData, deckName);
        const element = document.getElementById(elementId);
        if (element && foundDeck) {
            const count = countCardsInDeck(foundDeck);
...
function createDeckTreeItem(deck, path) {
    ...
    label.addEventListener('click', (e) => {
        ...
        } else if (cardCount > 0 && path.length > 0) {
            // Only allow selection of non-root decks
...
function showCard() {
    ...
    if (note.fields[0].includes('{{c')) {
        ...
        // Basic card
        const frontContent = processContent(note.fields[0]);
        const backContent = processContent(note.fields[1] || note.fields[0]);

        elements.cardFrontContent.innerHTML = frontContent;
        elements.cardBackContent.innerHTML = backContent;
    }
...
function processContent(html) {
    ...
    // Only remove GUIDs if they are the ONLY content (standalone)
    const stripped = html.replace(/<[^>]*>/g, '').trim();

    // Check if the entire content is just a GUID (various formats)
    // Matches: fdadde56253e4ba0a6f66f038e825662-ao-1, b8732455cdab419daef9fea4e0eff43b-oa-2, etc.
    if (/^[a-f0-9]{32}(-[a-z]{2,3}-\d+)?$/i.test(stripped) ||
        /^-[a-z]{2,3}-\d+$/i.test(stripped)) {
